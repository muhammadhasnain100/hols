...
export function PaymentSubnav() {
  ...
  return (
    <nav aria-label="Payment sections" className="flex flex-wrap gap-2.5">
      {LINKS.map((link) => {
        ...
        return (
          ...
          >
            <span className={cn("flex items-center justify-center", active ? "text-[#152744]" : "text-[color:var(--dash-muted)]")}>
              {link.icon}
            </span>
            {link.label}
          </Link>
